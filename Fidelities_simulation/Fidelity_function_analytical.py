...
def F_nzn(theta2, e1, e2, e3):
    """
    Computes:
    F = [ cos(e1)cos(e3)
          - (sin(e3)sin(e1)/4)*(3*cos(theta2) + cos(e2))
          + (cos(e1 - e3)/2)*sin(e2)
        ]^2
    """
    term1 = np.cos(e1) * np.cos(e3)  *np.cos(e2)
    term2 = (np.sin(e3) * np.sin(e1) / 4) * (3 * np.cos(theta2 +e2) + np.cos(e2))
    term3 = (np.sin(e1 + e3) / 2) * np.sin(e2)
...
def compute_n_z_n_fixed2():
    ...
    eps3 = np.linspace(*eps3_range, 150)
    E1, E3 = np.meshgrid(eps1, eps3)
    Z2 = F_nzn(theta2, E1, eps2_fixed, E3)
    # plot_surface_3d(E1, E3, Z2,
    #                 xlabel="ε1",
    #                 ylabel="ε3",
    #                 title=f"Infidelity 1-F(theta2={theta2:.2f}, eps2={eps2_fixed:.3f})")
...
def compute_n_z_n_fixed3():
    ...
    eps2 = np.linspace(*eps2_range, 150)
    E1, E2 = np.meshgrid(eps1, eps2)
    Z1 = F_nzn(theta2, E1, E2, eps3_fixed)
    # plot_surface_3d(E1, E2, Z1,
    #                 xlabel="ε1",
    #                 ylabel="ε2",
    #                 title=f"Infidelity 1-F(theta2={theta2:.2f}, eps3={eps3_fixed:.3f})")
...
def compute_different_theta():
    ...
    eps1_range = eps3_range = (-np.pi*2, np.pi*2)
    eps2_range = (np.pi*2, -np.pi*2)
    eps1 = np.linspace(*eps1_range, 150)
    eps2 = np.linspace(*eps2_range, 150)
    eps3 = np.linspace(*eps3_range, 150)
    E1, E2 = np.meshgrid(eps1, eps2)
